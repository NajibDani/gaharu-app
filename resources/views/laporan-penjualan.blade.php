                    ...
                    @forelse($groupedPesanans as $namaCustomer => $items)
                        <tr class="table-secondary customer-group-header" role="button" data-group="group-{{ $loop->index }}" data-customer-name="{{ strtolower($namaCustomer) }}">
                            ...
                            <td class="text-end pe-4 fw-bold text-success">
                                Rp {{ number_format($items->sum('details_sum_subtotal') - $items->sum('total_hpp'), 0, ',', '.') }}
                            </td>
                        </tr>
                        @foreach($items as $p)
                            <tr class="customer-group-row group-{{ $loop->parent->index }} d-none" data-kode="{{ strtolower($p->kode_pesanan) }}">
                                ...
                                <td class="text-end fw-bold text-dark">
                                    <div class="d-flex justify-content-end align-items-center gap-2">
                                        <span>Rp {{ number_format($p->details_sum_subtotal ?? 0, 0, ',', '.') }}</span>
                                        <button type="button" class="btn btn-link btn-sm p-0 btn-detail-harga-jual" data-id="{{ $p->id }}" data-type="b2b" title="Detail Harga Jual">
                                            <i class="bi bi-info-circle text-primary" style="font-size: 14px;"></i>
                                        </button>
                                    </div>
                                </td>
                                <td class="text-end pe-4 fw-bold text-success">
                                    Rp {{ number_format(($p->details_sum_subtotal ?? 0) - ($p->total_hpp ?? 0), 0, ',', '.') }}
                                </td>
                            </tr>
                        @endforeach
                    @empty
