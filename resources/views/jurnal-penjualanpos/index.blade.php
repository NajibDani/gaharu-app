                    ...
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><span class="badge bg-light text-dark border font-monospace px-2 py-1 small fw-bold">{{ $j->no_ref }}</span></td>
                                <td class="text-muted small">{{ Str::limit($j->deskripsi, 55, '...') }}</td>
                                <td class="text-end font-monospace text-success fw-semibold">Rp {{ number_format($j->total_debit, 2, ',', '.') }}</td>
                                <td class="text-end font-monospace text-danger fw-semibold">Rp {{ number_format($j->total_kredit, 2, ',', '.') }}</td>
                                <td class="text-center">
                                    <a href="{{ route('jurnal-penjualanpos.show', $j->id) }}" class="btn btn-sm btn-outline-info fw-bold px-2.5 py-1">Detail</a>
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="6" class="text-center text-muted py-5">Belum ada riwayat jurnal harian POS yang tersimpan.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
