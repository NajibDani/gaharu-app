...
<div class="alert alert-danger">
    <strong>Stok bahan baku tidak tersedia.</strong> Silakan lakukan pembelian terlebih dahulu.
</div>
@endif

<div class="card">
    <div class="card-header text-white fw-bold" style="background:#9c4f18; border-radius:24px 24px 0 0;">
    ...
    <div class="card-body p-4">
        <form method="POST" action="{{ route('pengeluaran-bahan-baku.store') }}">
            ...
            <div class="row mb-4">
                <div class="col-md-6">
                    ...
                    <select name="gudang_id" id="select-gudang" class="form-select @error('gudang_id') is-invalid @enderror" required>
                        ...
                        @foreach($gudang as $g)
                        <option value="{{ $g->id }}" data-kategori="{{ strtolower($g->kategori) }}" {{ (old('gudang_id', $selectedGudangId ?? '') == $g->id) ? 'selected' : '' }}>
                            {{ $g->nama }} ({{ $g->kategori }})
                        </option>
                        @endforeach
                    </select>
            ...
            <div class="mt-4">
                <button id="btnSimpan" type="submit" class="btn"
    ...
    if (rows.length === 1 && barangId !== '') {
        ...
        if (firstSelect && (!firstSelect.value || firstSelect.value === '')) {
            if (firstSelect.tomselect) {
                firstSelect.tomselect.setValue(barangId);
            } else {
...
document.addEventListener('DOMContentLoaded', function() {
    // Inisialisasi TomSelect untuk baris awal
    ...
    function applyAllSuggestions() {
        ...
        const firstSelect = firstRow.querySelector('.barang-select');
