...
<head>
    <meta charset="utf-8">
    <title>Surat Permintaan &amp; Transfer Bahan Baku - {{ $pengeluaran->kode_pengeluaran }}</title>
    <style>
        body {
...
<body>
    ...
    <div class="header">
        ...
        <p>Surat Permintaan &amp; Transfer Bahan Baku - Sistem Pengelolaan Stok &amp; Distribusi Antar Gudang</p>
    </div>
    ...
    <table class="table-info">
        ...
        <tr>
            ...
            <td class="val">: <strong>{{ $gudangUtama->nama ?? 'Gudang Utama' }}</strong></td>
    ...
    <table class="table-items">
        <thead>
            <tr>
                ...
                <th style="width: 80px;" class="text-end">Qty Diminta</th>
                <th style="width: 80px;" class="text-end">Stok Gudang Utama</th>
        ...
        <tbody>
            ...
            @forelse($pengeluaran->details as $index => $detail)
                ...
                <tr>
                    ...
                    <td class="fw-bold">{{ $detail->barang->nama ?? '-' }}</td>
                    <td class="text-end fw-bold">
                        {{ number_format($qtyDiminta, 2, ',', '.') }} <span class="text-muted" style="font-size: 8.5px;">{{ $satuan }}</span>
                    </td>
                    <td class="text-end">
                        {{ number_format($stokUtama, 2, ',', '.') }} <span class="text-muted" style="font-size: 8.5px;">{{ $satuan }}</span>
                    </td>
                    ...
                    <td class="text-end fw-bold">
                        Rp {{ number_format($detail->calculated_hpp ?? $detail->hpp_total ?? 0, 0, ',', '.') }}
                    </td>
                </tr>
            @empty
                <tr>
    ...
    <table class="footer-sign">
        <tr>
            <td>
                Pemohon / Peminta<br><br>
                <div class="sign-space"></div>
                ( __________________________ )<br>
                <small class="text-muted">{{ $pengeluaran->gudang->nama ?? 'Unit Pemohon' }}</small>
            </td>
            <td>
                Kepala Gudang<br><br>
                <div class="sign-space"></div>
                ( __________________________ )<br>
                <small class="text-muted">{{ $gudangUtama->nama ?? 'Gudang Utama' }}</small>
            </td>
            <td>
                Management<br><br>
                <div class="sign-space"></div>
